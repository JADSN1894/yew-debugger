...
use web_sys::{
    console::log_1, window, FocusEvent, HtmlElement, HtmlInputElement, InputEvent, KeyboardEvent,
    MouseEvent,
};
...
#[derive(Debug, PartialEq, YewMessage)]
pub enum Msg {
    CopyAll,
    PasteAll,
        ...
        match msg {
            Msg::BigInputFocused => {
                match self.input_cell.or(self.prev_focused_cell) {
                    Some(cell_id) => {
                        self.big_input_text =
                            self.inputs.get(&cell_id).cloned().unwrap_or_default();
            ...
            Msg::BigInputChanged { new_value } => match self.input_cell.or(self.focused_cell) {
                Some(cell_id) => {
                    ...
                    let expr =
                        parse(&new_value).unwrap_or_else(|_err| Expr::Str(new_value.clone()));
                ...
                Ok(inputs) => {
                    ...
                    for (cell_id, input) in &inputs {
                        match parse(&input) {
                            ...
                            Err(err) => {
                                return Err(format!(
                                    "cannot parse `{cell_id}` with `{input}` due to: {err:?}"
                                )
                                .into())
                            }
                        }
